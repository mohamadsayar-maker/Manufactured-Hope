...
function updateLabels() {
  ...
  els.statusText.textContent = horizontal
    ? 'Hold shutter and move up or down slowly.'
    : 'Hold shutter and move left or right slowly.';
}
...
function setupCanvases() {
  ...
  els.outputCanvas.width = size.width;
  els.outputCanvas.height = size.height;
  outCtx.fillStyle = '#050505';
  outCtx.fillRect(0, 0, size.width, size.height);
  els.liveCanvas.width = els.video.videoWidth || 1280;
  els.liveCanvas.height = els.video.videoHeight || 720;
...
function startScan(event) {
  ...
  els.shutterButton.classList.add('scanning');
  els.statusText.textContent = 'Scanning, keep moving slowly.';
...
function stopScan(event) {
  ...
  els.shutterButton.classList.remove('scanning');
...
function captureStrip() {
  ...
  if (els.liveCanvas.width !== videoW || els.liveCanvas.height !== videoH) {
    els.liveCanvas.width = videoW;
    els.liveCanvas.height = videoH;
  }

  liveCtx.drawImage(els.video, 0, 0, videoW, videoH);

  const mode = getMode();
  const slit = getSlitWidth();
  const step = Math.max(1, slit * getSpeed());
  const reverse = getDirection() === 'reverse';
  const glitch = getGlitch();

  if (mode === 'vertical') {
    const sourceX = Math.floor(videoW / 2 - slit / 2);
    const sourceY = 0;
    const sourceW = slit;
    const sourceH = videoH;
    const pasteX = reverse ? cursor - step : cursor;
    const jitterY = Math.random() < glitch ? Math.round((Math.random() - .5) * 34) : 0;
    const brightness = Math.random() < glitch ? 0.88 + Math.random() * .24 : 1;

    outCtx.save();
    outCtx.globalAlpha = brightness;
    outCtx.drawImage(els.liveCanvas, sourceX, sourceY, sourceW, sourceH, pasteX, jitterY, step, els.outputCanvas.height);
    outCtx.restore();

    cursor += reverse ? -step : step;
    const max = els.outputCanvas.width;
    updateProgress(reverse ? (max - cursor) / max : cursor / max);
    if ((!reverse && cursor >= max) || (reverse && cursor <= 0)) stopScan();
  } else {
    const sourceX = 0;
    const sourceY = Math.floor(videoH / 2 - slit / 2);
    const sourceW = videoW;
    const sourceH = slit;
    const pasteY = reverse ? cursor - step : cursor;
    const jitterX = Math.random() < glitch ? Math.round((Math.random() - .5) * 34) : 0;
    const brightness = Math.random() < glitch ? 0.88 + Math.random() * .24 : 1;

    outCtx.save();
    outCtx.globalAlpha = brightness;
    outCtx.drawImage(els.liveCanvas, sourceX, sourceY, sourceW, sourceH, jitterX, pasteY, els.outputCanvas.width, step);
    outCtx.restore();

    cursor += reverse ? -step : step;
    const max = els.outputCanvas.height;
    updateProgress(reverse ? (max - cursor) / max : cursor / max);
    if ((!reverse && cursor >= max) || (reverse && cursor <= 0)) stopScan();
  }
...
async function saveImage() {
  if (!scanHasImage) {
    els.statusText.textContent = 'Make a trace first.';
    return;
  }
  ...
  els.outputCanvas.toBlob(async blob => {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({ files: [file], title: 'TraceCam scan' });
        return;
      } catch (err) {
...
bindHoldButton(els.shutterButton, startScan, stopScan);

[els.modeSelect, els.directionSelect, els.slitWidth, els.speed, els.glitch].forEach(el => {
  el.addEventListener('input', updateLabels);
  el.addEventListener('change', () => { updateLabels(); if (!isScanning) retake(); });
});
